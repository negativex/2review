...
const getAllReviewId = async (id) => {
  const res = await fetch(`${process.env.DOMAIN_NAME}/api/reviews/${id}`, {
    cache: "no-store",
  });
  if (!res.ok) {
...
const getData = async (id) => {
  const res = await fetch(`${process.env.DOMAIN_NAME}/api/media/${id}`, {
    cache: "no-store",
  });
  if (!res.ok) {
    throw new Error("Failed to fetch data");
  ...
  return (
    <div>
      ...
      <div className="w-3/4 flex flex-col ml:5 lg:ml-10 xl:ml-20 2xl:ml-40 items-center">
        {adm_ids.length > 0 ? (
          adm_ids.map((id) => <OneReview key={id} review_id={id} />)
        ) : (
